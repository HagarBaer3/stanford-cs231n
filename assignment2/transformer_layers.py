...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        new_dim = self.head_dim
        H = self.n_head
        QX = self.query(query) # query: shape (N, S, E)
        KX = self.key(key)     # key: shape (N, T, E)
        VX = self.value(value) # Value : shape (N,T,E)
        
        #VX = VX.reshape(N, T, H, new_dim) #(N, T, H, newdim)
        #KX = KX.reshape(N, T, H, new_dim) #(N, T, H, newdim)
        #QX = QX.reshape(N, S, H, new_dim) #(N, S, H, newdim)
        ...
        if attn_mask is not None:
            mask = attn_mask[None, None, :, :]                        # (1,1,S,T) explicit broadcast
            X_soft = X_soft.masked_fill(~mask, float("-inf"))
