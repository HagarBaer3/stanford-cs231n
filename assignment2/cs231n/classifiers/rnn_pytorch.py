...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        #
        N, T, H, V = captions_in.shape[0], captions_in.shape[1], W_proj.shape[1],  W_vocab.shape[1]
        # (1) Use an affine transformation to compute the initial hidden state - (N,H)     
        # features - (N,D) , W_proj - (D,H)
        ...
        #W_embed - (V,W), captions(N, T+1)
        x_embed  = W_embed[captions_in]
        
        # (3) Use either a vanilla RNN or LSTM (depending on self.cell_type) to process the sequence of input word vectors and produce hidden state 
    ...
    def sample(self, features, max_length=30):
        ...
        # features (N, D),  W_proj (D,H) - > (N, H)
        h = torch.matmul(features, W_proj) + b_proj
       ...
       # for each row get the maximum score in the row
