...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        current_x = x[:,i,:]
        next_h = rnn_step_forward(current_x, prev_h, Wx, Wh, b)
        h[:,i,:], prev_h = next_h, next_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # all gates dim - (N,H)
    forget_gate = torch.sigmoid(f_ind)
    input_gate = torch.sigmoid(i_ind)
    out_gate = torch.sigmoid(o_ind)
    g_gate = torch.tanh(g_ind)
