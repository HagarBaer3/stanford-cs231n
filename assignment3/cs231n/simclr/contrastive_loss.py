...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        l_k = torch.exp(sim(z_k, z_k_N)/tau)
        sum1 = sum(torch.exp(sim(z_k, out[i])/tau) for i in range(2*N) if i != k)
        sum2 = sum(torch.exp(sim(z_k_N, out[i])/tau) for i in range(2*N) if i != k+N)
        total_loss -= torch.log(l_k/sum1) + torch.log(l_k/sum2)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs /= torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1)
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    sim_matrix = out @ out.T                            # 2Nx2N matrix
    norms = torch.linalg.norm(out, dim=1)               # Normal of each row - > (2N,)
    norms = norms.unsqueeze(1) * norms.unsqueeze(0)     # norms.unsqueeze(1) - (2N, 1), norms.unsqueeze(0) - (1,2N)--> (2N,2N)
    sim_matrix /= norms
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_idx = (torch.arange(2*N, device=out.device) + N) % (2*N)
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = None
    numerator = torch.exp(sim_matrix[torch.arange(2*N), pos_idx].unsqueeze(1) / tau)
