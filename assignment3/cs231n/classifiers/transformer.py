...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_embed_positional = self.embedding(captions)
        caption_embed_positional = self.positional_encoding(caption_embed_positional)
        
        # step 2 - prepare mask - for every a[i,j] that j > i is masked.
        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device)).bool()  
        # step 3 - Apply decoder features on the text and image embedding along the tgt_mask.
        memory = self.visual_projection(features).unsqueeze(1)
        scores = self.transformer(tgt = caption_embed_positional,memory = memory, tgt_mask= tgt_mask)
