...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        div_term = torch.exp(torch.arange(start=0, end=embed_dim, step=2, dtype=torch.float32)* (- torch.log(torch.tensor(10000.0)) / embed_dim))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        new_dim = self.emd_dim // self.n_head
        H = self.n_head
        QX = self.query(query) # query: shape (N, S, E)
        KX = self.key(key)     # key: shape (N, T, E)
        VX = self.value(value) # Value : shape (N,T,E)
        
        
        
        KX = KX.view(N, T, H, new_dim).transpose(1, 2).contiguous()   #(N, H, T, newdim)
        QX = QX.view(N, S, H, new_dim).transpose(1, 2).contiguous()   #(N, H, S, newdim)
        VX = VX.view(N, T, H, new_dim).transpose(1, 2).contiguous()   #(N, H, T, newdim)
        
        X_soft = torch.matmul(QX, KX.transpose(-2, -1))/ new_dim**0.5 #(N, H, S, newdim) x (N, H, newdim, T) = (N,H,S,T)                                 
        if attn_mask is not None:
            mask = attn_mask[None, None, :, :].bool() # Convert to bool explicitly
            X_soft = X_soft.masked_fill(mask == False, float("-inf"))
        
        attn_weights  = F.softmax(X_soft, dim=-1)               # shape: (N,H,S,T)
        attn_weights  = self.attn_drop(attn_weights)
        
        output = torch.matmul(attn_weights, VX)                # (N,H,S,T) x (N, H, T, newdim) = (N,H,S ,new_dim)
        
        output = output.transpose(1, 2).contiguous().view(N, S, E)      #(N,S,H,new_dim)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = patches.reshape(N,N_patches,patch_dim)
